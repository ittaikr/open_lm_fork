...
def evaluate(model, data, start_epoch, args, writer, average=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        with autocast():
            ...
            #assert( len(inputs) % args.val_freq == 0 )
            batch_size = math.ceil( len(inputs) / args.val_freq )

            for ii in range(args.val_freq):
                inputs_sub = inputs[ii * batch_size : (ii+1) * batch_size ,:]
                targets_sub =  targets[ii * batch_size : (ii+1) * batch_size ,:]
                out, _ = model(inputs_sub)  # [per_gpu_bs, seq_len, vocab_size]

                bs, seq_len = targets_sub.shape

                targets_sub = targets_sub.reshape(-1)
                total_loss = loss(out.reshape(-1, args.vocab_size), targets_sub)  # [bs * seq_len]

                # cross entropy ignores -100 values in loss computation
                mask = targets_sub != -100
    ...
    log_data["model"] = args.model # args.hf_model if args.hf_model else args.model

    return log_data


def evaluate_loop(model, data_list, start_epoch, args, writer, average_list=None):
    log_data_list = []

    if average_list is None:
        average_list = [None] * len(data_list)

    for i, (data, average) in enumerate(zip(data_list, average_list)):
        ...
        log_data_list.append(evaluate(model, data, start_epoch, args_copy, writer, average))
