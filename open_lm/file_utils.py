...
def log_num_checkpoints(total_steps, args):
    ...
    while steps_done < total_steps:
        ...
        steps_epoch = sum(
            [(n // (args.workers * args.batch_size * args.world_size)) * args.workers for n in num_samples_per_source]
        )
        steps_done += steps_epoch
        if steps_done > total_steps:
            steps_done = total_steps
        tokens_seen = steps_done * args.batch_size * args.world_size * args.seq_len
        checkpoints_made += 1
